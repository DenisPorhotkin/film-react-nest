...
export class MongoFilmsRepository extends FilmsRepository {
  ...
  async findById(id: string): Promise<FilmData | null> {
    const film = await this.filmModel.findOne({ id, isActive: { $ne: false } }).exec();
  ...
  async reserveSeat(filmId: string, scheduleId: string, row: number, seat: number): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`[${this.nameService}] Ошибка при бронировании места: ${error.message}`);
      return false;
    }
  }
  ...
  async releaseSeat(filmId: string, scheduleId: string, row: number, seat: number): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`[${this.nameService}] Ошибка при освобождении места: ${error.message}`);
      return false;
    }
  }
  ...
  async getAvailableSeats(filmId: string, scheduleId: string): Promise<{row: number, seat: number}[]> {
    ...
    if (!schedule) {
      this.logger.warn(`[${this.nameService}] Сеанс с ID ${scheduleId} не найден`);
      return [];
    }
